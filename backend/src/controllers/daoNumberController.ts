...
export async function getDaoTypes(req: Request, res: Response) {
  try {
    const result = await query(
      `SELECT code, libelle, description FROM dao_types ORDER BY code`
    );
...
export async function addDaoType(req: Request, res: Response) {
  try {
    ...
    const result = await query(
      ...
      [code.toUpperCase(), libelle, description || null]
    );
